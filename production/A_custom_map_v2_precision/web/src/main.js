...
const MODEL_URL = './models/A_custom_map_v2_low_ktx2.glb';
...
scene.add(new THREE.HemisphereLight(0x8ffcff, 0x03151a, 1.45));
const keyLight = new THREE.DirectionalLight(0xd9ffff, 2.8);
keyLight.position.set(-5, 9, 6);
scene.add(keyLight);
const rimLight = new THREE.PointLight(0x00d9ff, 6.5, 30, 2);
...
function setPartVisual(part, state) {
  ...
  part.traverse((child) => {
    ...
    const isOutline = child.name.startsWith('FX__OUTLINE_');
    if (state === 'selected') {
      material.emissive.setHex(isOutline ? 0x47fff1 : 0x00d9ff);
      material.emissiveIntensity = isOutline ? 3.1 : 0.72;
    } else if (state === 'hover') {
      material.emissive.setHex(isOutline ? 0x2cf0e3 : 0x00a9bd);
      material.emissiveIntensity = isOutline ? 2.5 : 0.45;
    } else {
...
function configureModel(root) {
  root.traverse((object) => {
    ...
    if (object.name.startsWith('FX__OUTLINE_')) outlines.push(object);
    if (object.isMesh) {
  ...
  metrics.outlines = outlines.length;
  buildRegionUI();
}
