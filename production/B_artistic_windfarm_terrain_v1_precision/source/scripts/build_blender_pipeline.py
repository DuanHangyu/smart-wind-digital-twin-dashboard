...
def clear_scene() -> None:
    ...
    scene.world.color = (0.018, 0.022, 0.023)
...
def terrain_pbr_material() -> bpy.types.Material:
    ...
    normal.inputs["Strength"].default_value = 0.94
...
def create_turbine(root: bpy.types.Object, data: dict, material: bpy.types.Material | None = None) -> bpy.types.Object:
    ...
    bpy.ops.mesh.primitive_cone_add(vertices=12, radius1=0.075, radius2=0.036, depth=tower_height, location=(0, 0, tower_height * 0.5))
    ...
    hub_height = tower_height + 0.04
    nacelle = create_box(f"MESH__NACELLE_{data['id']}", (0.22, 0.34, 0.15), (0, 0.05, hub_height), material)
    nacelle.parent = part
    link_only(nacelle, get_collection("COLL__TURBINES"))

    rotor = bpy.data.objects.new(f"ROTOR__TURBINE_{data['id']}", None)
    rotor.location = (0, -0.145, hub_height)
    rotor["rotor_axis"] = "Y"
    rotor["rpm"] = [8.2, 0.0, 9.1][int(data["id"]) - 1]
    rotor.parent = part
    get_collection("COLL__TURBINES").objects.link(rotor)

    bpy.ops.mesh.primitive_uv_sphere_add(segments=12, ring_count=6, radius=0.075, location=(0, 0, 0))
    ...
    for blade_index in range(3):
        ...
        length = radius * 0.90
        offset = radius * 0.10
        center_distance = offset + length * 0.5
        blade = create_box(
            f"MESH__BLADE_{data['id']}_{blade_index + 1}",
            (0.050, 0.026, length),
            (math.sin(angle) * center_distance, 0, math.cos(angle) * center_distance),
            material,
        )
        blade.rotation_euler.y = angle
        blade.parent = rotor
        link_only(blade, get_collection("COLL__TURBINES"))
    return part
...
def setup_preview(mode: str = "threequarter", wire: bool = False) -> None:
    ...
    floor_material = simple_material("MAT__PREVIEW_FLOOR", (0.003, 0.008, 0.009, 1), 0.83)
    ...
    if mode == "top":
        ...
        camera.location = (0, -18.0, 5.4)
        camera_data.lens = 62
        look_at(camera, Vector((0, 0.3, 0.85)))
    elif mode == "side":
        camera.location = (18.0, 0, 5.6)
        camera_data.lens = 62
        look_at(camera, Vector((0, 0.2, 0.85)))
    elif mode == "rear":
        camera.location = (-11.5, 14.5, 10.2)
        camera_data.lens = 58
        look_at(camera, Vector((0, 0.25, 0.65)))
    else:
        camera.location = (12.2, -15.0, 10.3)
        camera_data.lens = 58
        look_at(camera, Vector((0, 0.15, 0.68)))
    bpy.context.scene.camera = camera

    key_data = bpy.data.lights.new("Light__Key", "AREA")
    key_data.energy = 1450 if not wire else 390
    key_data.shape = "DISK"
    key_data.size = 8.0
    ...
    fill_data.energy = 610
    fill_data.color = (0.68, 0.78, 0.68)
    fill_data.size = 7.0
...
def main() -> None:
    ...
    (REPORT_DIR / "bake_manifest.json").write_text(
        json.dumps({
            "normal": {"method": "selected-to-active", "source": "257x193 high terrain", "target": "129x97 web terrain", "size": SPEC["texture_size"]},
            "ao": {"method": "selected-to-active", "source": "257x193 high terrain", "target": "129x97 web terrain", "size": SPEC["texture_size"]},
