...
def main():
    ...
    checks={
        ...
        "blade_radius_spread_le_1cm":max(radii)-min(radii)<=.01,"uv_in_unit_square":uv_min>=-1e-5 and uv_max<=1.00001,
    ...
    report={"blender_version":bpy.app.version_string,"parts":len(parts),"hotspots":len(hotspots),"rotors":len(rotors),"triangles":triangles,
            ...
            "axis_z_deviation_m":round(axis_z_deviation,5),"blade_radii_m":[round(x,4) for x in radii],"uv_range":[round(uv_min,6),round(uv_max,6)],"checks":checks,"pass":all(checks.values())}
