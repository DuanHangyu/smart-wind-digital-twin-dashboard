...
def create_blade(name: str, angle_degrees: float, quality: int) -> bpy.types.Object:
    # Reference-locked blade planform.  The V1 blade stopped widening too early and
    # read as a short propeller.  V2 keeps useful chord through the middle third,
    # then tapers over five stations to the slender wind-turbine tip.
    stations = [
        (.40, .22, .180, 24.0, .000, .000),
        (.58, .34, .165, 21.0, .008, -.008),
        (.88, .46, .145, 17.0, .025, -.018),
        (1.20, .45, .125, 13.0, .045, -.030),
        (1.55, .40, .105, 9.0, .070, -.045),
        (1.90, .34, .085, 6.0, .094, -.060),
        (2.25, .28, .067, 4.0, .112, -.078),
        (2.56, .22, .050, 2.4, .120, -.094),
        (2.82, .16, .036, 1.2, .116, -.108),
        (3.00, .105, .025, .5, .104, -.118),
        (ROTOR_R, .045, .014, 0.0, .086, -.124),
    ]
...
def create_capsule_shell(name: str, quality: int) -> bpy.types.Object:
    ...
    rings = [
        (-.36, .38, .42), (-.30, .56, .54), (-.18, .70, .62),
        (.05, .75, .64), (.60, .75, .64), (1.45, .75, .64),
        (2.02, .74, .63), (2.22, .68, .59), (2.36, .54, .49),
        (2.44, .31, .31), (2.47, .045, .045),
    ]
    vertices = []
...
def build_bedplate(q: int):
    objects = [
        add_cube("deck_left", (.46, 2.28, .16), (-.43, .94, 6.12), "silver_plate", .055, 3),
        add_cube("deck_right", (.46, 2.28, .16), (.43, .94, 6.12), "silver_plate", .055, 3),
        add_cube("bridge_front", (1.36, .38, .16), (0, -.01, 6.12), "silver_plate", .050, 3),
        add_cube("bridge_mid", (1.26, .28, .16), (0, .70, 6.12), "silver_plate", .045, 3),
        add_cube("bridge_rear", (1.36, .44, .16), (0, 1.88, 6.12), "silver_plate", .050, 3),
        add_cube("generator_pad", (1.02, .82, .14), (0, 1.57, 6.25), "silver_plate", .045, 3),
        add_cube("bearing_pad", (.78, .52, .12), (0, .24, 6.25), "silver_plate", .040, 3),
    ]
    for x in (-.56, .56):
        for y in (.04, .74, 1.43, 1.90):
...
def build_hub(q: int):
    ...
    objects = [add_sphere("hub_core", .40, HUB, "white_composite", (1, 1.05, 1), seg, seg // 2),
               add_torus("hub_front_seam", .35, .018, (0, HUB.y - .39, HUB.z), "rubber_seal", seg, 6,
                         Vector((0, 1, 0)))]
    for angle_deg in (0, 120, 240):
        angle = math.radians(angle_deg)
        direction = Vector((math.sin(angle), 0, math.cos(angle)))
        location = HUB + direction * .40
        objects += [
            add_cylinder(f"blade_socket_{angle_deg}", .21, .27, location, "white_composite", seg,
                         direction=direction, bevel=.018),
            add_torus(f"blade_seal_{angle_deg}", .19, .025, HUB + direction * .54, "rubber_seal", seg, 8,
                      direction=direction),
        ]
    return objects
...
def build_spinner(q: int):
    ...
    spinner = create_revolved_y("spinner", [
        (-1.36, .025), (-1.33, .10), (-1.25, .23), (-1.14, .34),
        (-1.00, .40), (-.84, .41), (-.72, .38),
    ], "white_composite", seg)
    return [spinner,
            add_torus("spinner_seam", .37, .017, (0, -.76, HUB.z), "rubber_seal", seg, 6,
                      Vector((0, 1, 0)))]
...
def build_main_bearing(q: int):
    seg = 40 if q >= 2 else 28
    return [
        add_torus("bearing_outer_ring", .235, .090, (0, .22, 6.55), "blue_machine", seg, 10,
                  Vector((0, 1, 0))),
        add_torus("bearing_inner_seal", .137, .024, (0, .205, 6.55), "rubber_seal", seg, 8,
                  Vector((0, 1, 0))),
        add_cube("bearing_pedestal", (.66, .44, .28), (0, .22, 6.30), "blue_machine", .060, 3),
        add_cube("bearing_foot_l", (.29, .52, .10), (-.25, .22, 6.23), "blue_machine", .025, 2),
        add_cube("bearing_foot_r", (.29, .52, .10), (.25, .22, 6.23), "blue_machine", .025, 2),
    ]
...
def build_gearbox(q: int):
    seg = 40 if q >= 2 else 28
    objects = [
        add_cube("gearbox_body", (.70, .58, .58), (0, .76, 6.58), "blue_machine", .105, 5),
        add_cube("gearbox_lower", (.74, .56, .24), (0, .76, 6.35), "blue_machine", .060, 3),
        add_cube("gearbox_cap", (.54, .46, .20), (0, .76, 6.94), "blue_machine", .055, 3),
        ...
        add_torus("gearbox_input_seal", .14, .022, (0, .325, 6.55), "rubber_seal", seg, 8,
                  Vector((0, 1, 0))),
        add_cylinder("gearbox_output_collar", .17, .17, (0, 1.10, 6.55), "blue_machine", seg,
                     Vector((0, 1, 0)), .014),
    ]
    for x in (-.26, .26):
...
def build_generator(q: int):
    seg = 40 if q >= 2 else 28
    objects = [
        add_cube("generator_body", (.78, .82, .72), (0, 1.58, 6.61), "blue_machine", .105, 5),
        add_cylinder("generator_front", .30, .16, (0, 1.10, 6.58), "blue_machine", seg,
                     Vector((0, 1, 0)), .018),
        add_cylinder("generator_rear", .32, .10, (0, 2.04, 6.58), "blue_machine", seg,
                     Vector((0, 1, 0)), .014),
        add_cube("generator_terminal", (.55, .55, .21), (0, 1.58, 7.04), "blue_machine", .05, 3),
    ...
    for side in (-1, 1):
        for i, z in enumerate((6.40, 6.54, 6.68, 6.82)):
            objects.append(add_cube(f"generator_fin_{side}_{i}", (.035, .62, .045),
                                    (side * .405, 1.58, z), "blue_machine", .008, 1))
    return objects
...
def build_assembly(quality: int, mode: str, shell_glass: bool = False, exploded: float = 0.0,
    ...
    root["asset_id"] = "C_dismantlable_wind_turbine_v2_reference_refined"
    root["units"] = "meter"
    root["up_axis"] = "+Z"
    root["rotor_axis"] = "+Y"
    ...
    for canonical in [item["name"] for item in SPEC["parts"]]:
        if canonical == "PART__NACELLE_SHELL":
            ...
            front_seam = add_torus("nacelle_front_seam", .66, .012, (0, -.20, 6.60),
                                   "silver_plate", seg, 6, Vector((0, 1, 0)))
            front_seam.scale.x, front_seam.scale.z = 1.06, .94
            bpy.context.view_layer.objects.active = front_seam
            bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
            rear_seam = add_torus("nacelle_rear_seam", .57, .012, (0, 2.30, 6.60),
                                  "silver_plate", seg, 6, Vector((0, 1, 0)))
            rear_seam.scale.x, rear_seam.scale.z = 1.10, .92
            bpy.context.view_layer.objects.active = rear_seam
            bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
            objects += [front_seam, rear_seam,
                        add_cube("nacelle_side_seam_l", (.012, 2.08, .018), (-.751, 1.03, 6.35),
                                 "silver_plate", .002, 1),
                        add_cube("nacelle_side_seam_r", (.012, 2.08, .018), (.751, 1.03, 6.35),
                                 "silver_plate", .002, 1)]
...
def setup_scene(camera_mode: str = "full", transparent=False) -> None:
    ...
    positions = {
        "full": ((14.5, -22.0, 8.6), (0, .25, 4.42), 55),
        "front": ((0, -23.0, 5.25), (0, -.15, 4.48), 55),
        "side": ((21.0, .45, 5.45), (0, .45, 4.48), 55),
        "rear": ((0, 23.0, 5.25), (0, .45, 4.48), 55),
        "top": ((10.0, -13.0, 17.0), (0, .25, 4.4), 52),
...
def export_glb(root: bpy.types.Object) -> Path:
    ...
    path = EXPORT_DIR / "C_dismantlable_turbine_v2_low_raw.glb"
