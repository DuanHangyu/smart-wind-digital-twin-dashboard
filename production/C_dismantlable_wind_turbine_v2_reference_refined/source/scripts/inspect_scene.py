...
def main():
    ...
    checks={
        ...
        "blade_radius_reference_target_3_10m":all(3.08 <= radius <= 3.13 for radius in radii),
        "rotor_radius_to_tower_height_reference_ratio":.53 <= (sum(radii)/len(radii))/5.72 <= .56,
    ...
    report={"blender_version":bpy.app.version_string,"parts":len(parts),"hotspots":len(hotspots),"rotors":len(rotors),"triangles":triangles,
            ...
            "rotor_radius_to_tower_height_ratio":round((sum(radii)/len(radii))/5.72,4),
