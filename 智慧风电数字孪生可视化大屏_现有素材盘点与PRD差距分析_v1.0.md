...
| PRD 模块 | 状态 | 具体缺口 |
|---|---|---|
| 风场页完整三栏布局 | W1/W2 已完成 | 环境、历史功率、排行、设备表和发电详情已经接入统一大屏壳层 |
| 左侧环境罗盘 | W2 已完成 | 风向、温度、湿度、气压和风速由 1 Hz 模拟数据驱动 |
| 左侧历史功率 | W2 已完成 | 复用统一折线组件并持续更新 |
| 左侧条形排行 | W2 已完成 | 自动循环，悬停暂停 |
| `主风机A/B/D` 命名 | W4 已完成 | `PART__TURBINE_01/02/03` 映射为 `T-A01/T-A02/T-A04` |
| 风机状态样式 | W4 已完成 | 热点、锚点卡和设备表统一正常/待机/异常/故障/离线状态 |
| `水面`图层按钮 | W4 已完成 | 直接控制真实 `PART__LAKE` 及湖泊热点显隐 |
| `投影`按钮 | W4 已完成 | 同一相机下切换真实地形三角线框 |
| `电路`图层 | 按 V1 约束禁用 | 缺少业务线路拓扑时保持明确的禁用态 |
| 风机信息弹窗 | W4 已完成 | GLB 热点锚定，显示实时功率、风速、状态，支持关闭和空白取消 |
| 被选风机视觉关联 | W4 已完成 | 模型材质高亮、热点激活、锚点引线卡和设备表选中态同步 |
| 右侧设备状态表 | W2/W4 已完成 | A001–A010 循环滚动，模型已有的三台设备支持反向选择 |
| 右侧发电详情 | W2 已完成 | 双系列柱图已接入 |
| 风机离线行为 | W4 已完成 | 三台风机按 GLB RPM 持续旋转；离线叶轮停止，标签变色，缺失数据展示 `--` |
| 相机巡航暂停/恢复 | W4 已完成 | 用户操作时暂停，结束 5 秒后恢复 |
| 3D 加载百分比/静态降级 | W4 已完成 | 真实加载进度、WebGL 检测和最终渲染静态图降级 |
...
W2 已建立统一页面状态：P01 风机/地标/区域轮播，P02 水面/投影/电路占位，P03 四模式互斥与独立动画开关；状态使用浏览器本地存储持久化，所有定时器和动画帧均在组件卸载时清理。W3 已接通 P01，W4 已接通 P02；只剩 W5 将 P03 状态连接到真实 Three.js 场景。
