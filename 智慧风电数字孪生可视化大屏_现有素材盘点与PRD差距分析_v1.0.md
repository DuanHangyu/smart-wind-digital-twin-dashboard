...
目前已经完成网站最困难的三类核心三维资产：A 自定义区域地图、B 艺术化风场地形、C 可拆解风机。三套资产均包含 Blender 分阶段源文件、网页低模、UV、PBR 贴图、KTX2、GLB、交互节点、Three.js 演示代码和独立性能报告，可以进入大屏集成阶段。
...
| PRD 模块 | 状态 | 具体缺口 |
|---|---|---|
| 统计页完整三栏布局 | 缺失 | 当前只有资产演示页，没有 PRD 左中右布局 |
| 顶部天气/标题/时间 | 缺失 | 需全局壳层提供 |
| 左侧风机详情 | 缺失 | KPI、风机全息插画、数字补间均未实现 |
| 左侧双柱图 | 缺失 | 需短横块发光柱体组件 |
| 左侧环图 | 缺失 | 需四状态环、图例和动画 |
| 中央业务标题/总发电量 | 缺失 | 需绑定页面数据，而非模型验收指标 |
| 广西 14 地市名称 | 待确认 | 当前为北辰、云岭、西原等自定义名称 |
| 风机分布标志 | 缺失 | A 模型没有地图层的风机图标/点位 |
| 地标定位点 | 部分 | 有区域标签，无 PRD 定位点样式和开关 |
| 外围圆弧/点阵/南部光点 | 缺失 | 建议 Three.js 程序化实现，不需要重新建模 |
| 底部不规则垂直光柱 | 部分 | 有挤出侧壁，但缺参考图中的光柱动画层 |
| 风机/地标/动画按钮 | 缺失 | 当前只有复位和自动巡航 |
| 区域自动轮播高亮 | 缺失 | 当前需手动选择，没有 0.6–1.2 秒轮播状态机 |
| 右侧条形排行 | 缺失 | 需 TOP 列表和循环滚动 |
| 右侧折线图 | 缺失 | 需 ECharts/Canvas 图表 |
| 右侧设备预警 | 缺失 | 需待处理/已处理卡片 |
| 加载/空态/接口失败 | 部分 | 只有模型加载失败；面板级状态缺失 |
...
当前这些文件均不存在。模型内的 glTF extras 只适合作为空间和节点元数据，不能代替页面业务数据。
...
当前三个演示页各自维护局部变量，没有统一页面状态、持久化和卸载策略。
...
| 编号 | 问题 | 不确认的影响 | 建议默认值 |
|---|---|---|---|
| Q1 | P01 使用自定义 14 区名称，还是改成广西 14 地市？ | 影响地图文案、数据和视觉验收 | 视觉复刻优先时改为广西名；模拟产品优先时保留自定义 |
